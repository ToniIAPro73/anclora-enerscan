    ...
    <>
      ...
      {isOpen ? (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/45 px-4 py-6 backdrop-blur-sm sm:items-center" role="dialog" aria-modal="true" aria-labelledby="cookie-consent-title">
          <div className="w-full max-w-lg rounded-3xl border border-[#00DC82]/20 bg-[#07130f] p-6 text-white shadow-2xl">
            {!showSettings ? (
              <div className="space-y-5">
                <div>
                  <p className="mb-2 text-xs font-semibold uppercase tracking-[0.24em] text-[#00DC82]">Anclora Group</p>
                  <h2 id="cookie-consent-title" className="font-heading text-2xl font-semibold">{t.cookiesTitle}</h2>
                  <p className="mt-3 text-sm leading-6 text-white/70">{t.cookiesDescription}</p>
                </div>
                <div className="flex flex-col gap-3 sm:flex-row">
                  ...
                  <button type="button" onClick={() => setShowSettings(true)} className="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold text-white transition hover:border-[#00DC82]/60">{t.cookiesSettings}</button>
                  <button type="button" onClick={() => persist(defaultPreferences)} className="rounded-full px-5 py-3 text-sm font-semibold text-white/70 transition hover:text-white">{t.cookiesRejectOptional}</button>
                </div>
              </div>
            ) : (
              <div className="space-y-5">
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:justify-between">
                  <button type="button" onClick={() => setShowSettings(false)} className="rounded-full px-5 py-3 text-sm font-semibold text-white/70 transition hover:text-white">{t.cookiesBack}</button>
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <button type="button" onClick={() => persist(defaultPreferences)} className="rounded-full border border-white/15 px-5 py-3 text-sm font-semibold text-white transition hover:border-[#00DC82]/60">{t.cookiesRejectOptional}</button>
